using System;
using System.Collections.Generic;
using System.Text;

namespace Entities
{
    public class E_PerfilComp
    {
        public string Codigosap { set; get; }
        public string Codperfilcomp { set; get; }
        public string Descripcionsap { set; get; }
        public string Fechacreacion { set; get; }
        public string Fechamodificacion { set; get; }
        public bool Flagactivo { set; get; }
        public string Hostcreacion { set; get; }
        public string Hostmodificacion { set; get; }
        public int Idestadopc { set; get; }
        public int Idperfil { set; get; }
        public int Idperfilcomp { set; get; }
        public int Idperfilcomppadre { set; get; }
        public int Idusuariocreacion { set; get; }
        public int Idusuariomodificacion { set; get; }
        public string Perfilcomp { set; get; }
    }
}
